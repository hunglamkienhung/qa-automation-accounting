...
- `@module:NN-name`, and a tier tag: `@be`/`@fe` with `@db`/`@api`, plus a
  target tag (`@books`, `@nager`) and a domain sub-tag (`@journal`,
  `@reports`, `@integrity`, `@security`). The two stacks select the same set —
...
```gherkin
But "a future public holiday not yet announced" cannot be verified because
    "Nager.Date lists only holidays already published, not ones to come"
```
